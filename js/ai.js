...
(function (root, factory) {
  ...
  // ---------- 叫地主 ----------
  // 返回 { bid: 0|1|2|3, reason }；minBid 为当前已有最高叫分（须叫得更高才有效）
  function decideBid(handRanks, difficulty, seat, minBid) {
    var s = handStrength(handRanks);
  ...
  function moveScore(ranks) {
    ...
    return base + (typePenalty[parsed.type] || 0);
  }
  ...
  function leadCandidates(handRanks) {
    var moves = C.generateMoves(handRanks);
    var cands = [];
    moves.forEach(function (m) {
      ...
      cands.push({ ranks: m, score: moveScore(m) });
    });
    if (!cands.length) {
  ...
  function pickBestLead(handRanks, opts) {
    opts = opts || {};
    var sorted = leadCandidates(handRanks);
    if (opts.variety === false) return sorted[0] || null;
    return variedPick(sorted, opts.rng);
  }

  // 跟牌候选（升序打分）
  function followCandidates(handRanks, givenRanks) {
    var beats = C.findBeats(handRanks, givenRanks);
    if (!beats.length) return [];
    var given = C.parseCards(givenRanks);
    var cands = [];
  ...
  function pickBestFollow(handRanks, givenRanks, opts) {
    opts = opts || {};
    var sorted = followCandidates(handRanks, givenRanks);
  ...
  function shouldPass(handRanks, givenRanks, opponentCount, difficulty) {
    var beats = C.findBeats(handRanks, givenRanks);
    if (!beats.length) return true;
    var parsed = C.parseCards(givenRanks);
    var level = DIFFICULTY[difficulty] || 0;
  ...
  // 主入口：根据局面出牌
  // ctx: { handRanks, lastPlay(或null), lastPlayerIndex, myIndex, playersCount, difficulty, teammateIndex, seat, bidInfo }
  ...
  function decidePlay(ctx) {
    ...
    if (last && ctx.lastPlayerIndex !== ctx.myIndex) {
      ...
      var pass = shouldPass(hand, last, ctx.playerCounts[ctx.lastPlayerIndex], ctx.difficulty);
      if (pass) return null;
      return pickBestFollow(hand, last, { rng: seatRng });
    } else {
      // 首出/自由出
      return pickBestLead(hand, { rng: seatRng });
    }
  }
